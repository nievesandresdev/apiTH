        ...
        foreach ($stays as $stay) {
            ...
            foreach ($stay->queries as $query) {
                ...
                //$urlQr = "https://thehosterappbucket.s3.eu-south-2.amazonaws.com/test/qrcodes/qr_nobuhotelsevillatex.png";
                    ...
                    $urlPrivacy = buildUrlWebApp($chainSubdomain, $stay->hotel->subdomain,'privacidad',"e={$stay->id}&g={$query->guest->id}&email=true");
                    $queryData = [
        ...
        foreach($stays as $stay){
            ...
            foreach ($stay->guests as $guest) {
                if(!$guest->off_email){
                    ...
                    Log::info("No se envía correo postCheckin email_off a {$guest->email} (Estancia ID: {$stay->id}, Hotel: {$stay->hotelName})");
                }
            }
